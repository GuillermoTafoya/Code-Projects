...
def runGUI(filename,MASTER):
    ...
    titleScreen = Frame(root, width=620, height=400)
    ...
    generatePassword = Frame(root, width=400, height=300)

    for frame in (getPasswordScreen, addAccountScreen, deleteAccountScreen, changePassword, displayAccounts,generatePassword):
    ...
    barWelcome = ImageTk.PhotoImage(Image.open("BarWelcome.jpg").resize((620, 400), Image.ANTIALIAS))
    barBackground = ImageTk.PhotoImage(Image.open("BarBackground.jpg").resize((620, 400), Image.ANTIALIAS))
    ...
    def getPassword():
        ...
        try:
            GUI.getPasswordFromAccounts(masterPassword.value, str(quickPassAccess.get()),int(quickKey1Access.get()),int(quickKey2Access.get()),passwords)
        except:
    ...
    def changePasswordMethod(masterPassword):
        try:
            masterPassword.setNew(GUI.changePassword(masterPassword.value, str(changePasswordAccount.get()),int(changePasswordKey1.get()),int(changePasswordKey2.get()),passwords,str(newPassword.get()),filename))
        except:
            print("\nEntered keys and/or password are not valid.") 
    ...
    centered=0.35
    Button(changePassword, text = "Change password" , command = lambda: changePasswordMethod(masterPassword)).place(relx = centered , rely = 0.7, relheight = 0.07, relwidth = 0.3)
    ...
    def addPasswordMethod():
        try:
            GUI.addAccount(masterPassword.value, str(addAccountAccount.get()),int(addAccountKey1.get()),int(addAccountKey2.get()),passwords,str(addAccountNewPassword.get()),filename)
        except:
            print("\nEntered keys and/or password are not valid.") 
    ...
    def deleteAccountMethod():
        try:
            GUI.deleteAccount(str(deleteA.get()),passwords,filename)
        except:
    ...
    newPSBG = Label(generatePassword, image=barBackground)
    newPSBG.image = barBackground
    newPSBG.place(x=0, y=0)

    def setGeneratetPasswordsScreenDefaultText(newPassDT):
        newPass.delete(0,END)
        newPass.insert(0, newPassDT)


    def makeNewPassword():

        try:
            generatedPassword = GUI.generateNewPassword(int(newPass.get()))
    ...
    newPassDT = "Length of password"

    newPass = Entry(generatePassword)

    centered=0.35
    Button(generatePassword, text = "New password" , command = lambda: makeNewPassword()).place(relx = centered , rely = 0.4, relheight = 0.07, relwidth = 0.3)


    newPass.place(relx = centered , rely = 0.2, relheight = 0.07, relwidth = 0.3)


    newPass.bind("<Button-1>", lambda do: clearSearch(do,newPassDT))
    ...
    Button(searchBar, text = "New Password" , command = lambda: combineFuncs(swap(generatePassword),setGeneratetPasswordsScreenDefaultText(newPassDT))).grid(row=0, column=6)
